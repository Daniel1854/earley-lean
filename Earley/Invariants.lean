...
The proofs for the EarleySet follow the work from Rau et Nipkow:
...
TODO: introduce some Prop that revolves around G = G'
-/
namespace Fixpoint
...
public theorem soundnessEarleyFixpoint {G : ContextFreeGrammar T} [BEq T] [BEq G.NT]
    [LawfulBEq G.NT] (w : List T) {Gₗ : ContextFreeGrammarList T G.NT} (h : CFG_eq_CFGₗ G Gₗ)
    (hw : isWord G (w.map Symbol.terminal)) (hgen : G.Generates (w.map Symbol.terminal)) :
...
public theorem EarleySet_eq_earleyFixpoint {G : ContextFreeGrammar T} [BEq T] [BEq G.NT]
    ...
    (hw : isWord G (w.map Symbol.terminal)) :
    EarleySet G (w.map Symbol.terminal) = earleyFixpoint Gₗ w := by
  have hsub1 := EarleySet_sub_earleyFixpoint w h hw
  have hsub2 := earleyFixpoint_sub_EarleySet w h hw
...
open Earley.Invariants.Model
open Earley.Fixpoint
open Earley.Invariants.Fixpoint
open Recognizer
open Utils
open ContextFreeRule
open ContextFreeGrammar
